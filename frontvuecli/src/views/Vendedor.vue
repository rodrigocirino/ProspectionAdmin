  ...
  mounted() {
    ...
    if (this.$store.state.logged !== true) {
      if (this.$router.history.current.path !== '/login') {
        //Desativado porque ele precisa cadastrar para fazer o login
  ...
  methods: {
    ...
    onRowSelected(items) {
      ...
      if (this.selected) {
        ...
        this.senha = this.selected.senha;
        ...
        nome: this.nome, email: this.email, senha: this.senha
          ...
          .catch(error => {
            ...
            if (error.response && error.response.status == 304) {
              this.submitStatus = 'Registro com restrições, cliente possui prospectos abertos !';
            }
          });
    },
    remover() {
      this.axios.delete('/vendedor', {
        ...
        data: {
          ...
          .then(response => {
            if (response.status === 200) {
              ...
              this.submitStatus = 'Registro com restrição de remoção, possui prospectos abertos !';
            }
            this.listar();
            this.resetForm();
          })
